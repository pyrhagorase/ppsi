...
<body>
   ...
   <main class="main-content">
        <div style="margin-bottom: 30px;">
            ...
            <form id="notaForm" method="POST" action="/owner/nota/simpan">
                @csrf
                <!-- Modal Nota -->
    ...
    <script>
        ...
        function ubahStatus() {
            ...
            fetch("{{ route('owner.updateServisStatus', ['id_tracking' => ':trackingId']) }}".replace(':trackingId', trackingId), {
                    method: 'POST',
                    headers: {
                    ...
                    if (data.success) {
                        ...
                        switch (selectedStatus) {
                            case "KonfirmasiBiaya":
                                window.location.href = "{{ route('owner.konfirmasibiaya') }}";
                                break;
                            case "Diproses":
                                window.location.href = "{{ route('owner.diproses') }}";
                                break;
                            case "Selesai":
                                window.location.href = "{{ route('owner.selesai') }}";
                                break;
                            case "Lunas":
                                window.location.href = "{{ route('owner.lunas') }}";
                                break;
                            default:
                                window.location.href = "{{ route('owner.daftarservis') }}";
                                break;
                        }
                    } else {
        ...
        document.getElementById("editBtn").addEventListener("click", function() {
            ...
            fetch("{{ route('owner.updateKeterangan', ['id_tracking' => ':trackingId']) }}".replace(':trackingId', trackingId), {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        document.getElementById("updateDataBtn").addEventListener("click", function() {
            ...
            fetch("{{ route('owner.updateServisDetail', ['id_tracking' => ':trackingId']) }}".replace(':trackingId', trackingId), {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute("content")
        ...
        document.getElementById("deleteDataBtn").addEventListener("click", function() {
            ...
            fetch("{{ route('owner.deleteServis', ['id_tracking' => ':trackingId']) }}".replace(':trackingId', trackingId), {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Content-Type': 'application/json',
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        window.location.href = "{{ route('owner.daftarservis') }}"; // redirect setelah delete
                    } else {
        ...
        document.getElementById('notaBtn').addEventListener('click', function() {
            ...
            if (userEmail) {
                ...
                document.getElementById('notaKasir').value = 'Owner'; // Default jika tidak ditemukan
            }
